...
const PostPropertyThree: React.FC<stepThreeProps> = ({
  ...
  const discription = useSelector(
    (state: any) => state?.PostpropertyReducer?.discription
  );
  ...
  const generateAiDescription = () => {
    dispatch(
      postpropertyAction.fetchAutoGeneratediscription(
        `use this object ${JSON.stringify({
          // property_owner: propertyOwner,
          floor_type: totalData.floor_type,
          ownership: stepThreeData.ownership,
          expected_price: stepThreeData.expected_price,
          Property_for: totalData.iwant,
          property_type: totalData.property_type,
          bhk: totalData.bhk,
          balconies: totalData.balconies,
          bathrooms: totalData.bathrooms,
          floor_no: totalData.floor_no,
          total_floors: totalData.total_floors,
          facing: totalData.facing,
          property_age: totalData.property_age,
          water_source: totalData.water_source,
          city: totalData.city,
          building_name: totalData.building_name,
          state: totalData.state,
          pincode: totalData.pincode,
          landmark: totalData.landmark,
        })}  and generate a property description and location description 
        with heading Like Property description and location description in bullet in 100 words.`
      )
    );
  };
      ...
      <div>
        ...
        <div className="property__description__container">
          {dicriptionLoader ? (
            ...
            <p style={{ cursor: "pointer" }} onClick={generateAiDescription}>
              Generate With Ai
            </p>
          )}
          ...
          <textarea
            ...
            value={discription || stepThreeData.property_description}
            onChange={(e) => {
              setStepThreeData({
                ...stepThreeData,
                property_description: discription || e.target.value,
              });
            }}
          />
        </div>
